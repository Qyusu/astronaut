...
parse_client = OpenAIChatClient(api_key=settings.OPENAI_API_KEY, default_model_version="gpt-4o-mini-2024-07-18")

THINKING_MODEL_MAX_TOKENS = 64000
DEFAULT_MODEL_MAX_TOKENS = 8192
DEFAULT_THINKING_BUDGET_TOKENS = 20000
...
class AnthropicChatClient(BaseLLMClient):
    ...
    def _parse_response(self, response: Message | list[Message], response_format: Optional[Type[BaseModel]]) -> str:
        try:
            if isinstance(response, list):  # for thinking model
                stream_content = []
                for message in response:
    ...
    def parse_chat(
        ...
        max_retries: int = 3,
        max_thinking_tokens: int = DEFAULT_THINKING_BUDGET_TOKENS,
        ...
        Args:
            ...
            n (Optional[int], optional): Number of completions. Note: This parameter
            ...
            max_thinking_tokens (int, optional): Maximum number of tokens to use
        ...
        while True:
            try:
                if model_version in ANTHOROPIC_THINKING_SERIES:
                    max_tokens = max_tokens or THINKING_MODEL_MAX_TOKENS
