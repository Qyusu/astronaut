...
class GoogleChatClient(BaseLLMClient):
    ...
    def _get_token_count(self, response: GenerateContentResponse) -> tuple[int, int, int]:
        ...
        input_tokens = usage.model_dump().get("prompt_token_count", 0)
        cached_tokens = usage.model_dump().get("cached_content_token_count", 0)
        output_tokens = usage.model_dump().get("candidates_token_count", 0)
