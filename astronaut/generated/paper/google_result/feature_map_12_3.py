...
class LEFM_FRZFeatureMap(BaseFeatureMap):
    """LEFM with Final RZ layer (LEFM-FRZ) class.
    ...
    def __init__(self, n_qubits: int, a_coeffs: np.ndarray = None, c_coeffs: np.ndarray = None) -> None:
        """Initialize the LEFM with Final RZ layer feature map class.

        Args:
            n_qubits (int): number of qubits
            a_coeffs (np.ndarray): Coefficients for the RX rotations. Default is np.ones(n_qubits).
            c_coeffs (np.ndarray): Coefficients for the RZ rotations. Default is np.ones(n_qubits).
        """
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        for layer in range(n_layers):
            ...
            # Final RZ Encoding Layer
            for i in range(self.n_qubits):
                qml.RZ(
                    phi=self.c_coeffs[i] * x[layer * 3 * self.n_qubits + 2 * self.n_qubits + i],
                    wires=i,
                )
