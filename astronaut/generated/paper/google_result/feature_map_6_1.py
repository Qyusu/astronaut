...
class LEFM_NLPERYFeatureMap(BaseFeatureMap):
    """LEFM with Non-linear Post-Entanglement RY (LEFM-NLPERY) class.
    ...
    def __init__(self, n_qubits: int, a_coeffs: np.ndarray = None, b_coeff: float = 1.0) -> None:
        """Initialize the LEFM with Non-linear Post-Entanglement RY feature map class.

        Args:
            n_qubits (int): number of qubits
            a_coeffs (np.ndarray): Coefficients for RX rotations. Default is np.ones(n_qubits).
            b_coeff (float): Coefficient for the non-linear function in RY rotations. Default is 1.0.
        """
