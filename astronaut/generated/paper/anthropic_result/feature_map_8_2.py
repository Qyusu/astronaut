...
class BalancedRepetitionEncoder(BaseFeatureMap):
    """Balanced Repetition Encoder with Consistent Phases.
    
    This feature map focuses on maintaining consistency across repetitions 
    while preserving the effective three-layer entanglement structure.

    Args:
        BaseFeatureMap (_type_): base feature map class

    Example:
        >>> feature_map = BalancedRepetitionEncoder(n_qubits=10)
    """

    def __init__(self, n_qubits: int, scale_factor: float = np.pi, offset: float = np.pi/4) -> None:
        """Initialize the Balanced Repetition Encoder.
    ...
    def _apply_phase1(self) -> None:
        """Apply phase shifts after Layer 1: Rz(π/4) to all qubits."""
        for i in range(self.n_qubits):
            qml.RZ(phi=np.pi/4, wires=i)

    def _apply_phase2(self) -> None:
        """Apply phase shifts after Layer 2: Rz(π/3) to all qubits."""
        for i in range(self.n_qubits):
