...
class HybridCPSMidRangeWithDynamicCalibrationFeatureMap(BaseFeatureMap):
    """
    Hybrid CPS Mid-Range with Dynamic Calibration Feature Map.
    
    This feature map partitions an 80-dimensional PCA-reduced input into 5 layers (each with 16 features).
    For each layer (l = 0,...,4):
      
    - Local Encoding:
         The first 10 features are encoded onto a 10-qubit register via fixed RY(π*x) rotations, ensuring a linear, non-trainable mapping that preserves local amplitude information.
    ...
    - Stage 1 (Immediate Neighbor Entanglement):
         ...
         A CRX gate is applied between qubit j and (j+1 mod n_qubits) with rotation angle computed as:
             π * (0.5*x_a + 0.5*x_b + ε*(x_a - x_b)),
         where ε is a modest contrast coefficient (default 0.05) that can be dynamically recalibrated.
    ...
    - Stage 2 (Next-Nearest Neighbor Entanglement):
         ...
         A CRY gate is applied between qubit j and (j+2 mod n_qubits) with rotation angle π times the average value.
      
    - Stage 3 (Mid-Range Entanglement via CPS):
         For each qubit j, using the same pair of auxiliary features as in Stage 1, a CPS (ControlledPhaseShift) gate is applied between qubit j and (j+3 mod n_qubits).
         The rotation angle is given by:
             π * λₗ * (0.5*x_a + 0.5*x_b),
         where λₗ is an adaptive linear scaling factor for the layer, provided via the parameter lambda_values.
      
    - Global Fusion:
         After processing all layers, a MultiRZ gate aggregates inter-layer features with a rotation angle:
             π * Σₗ [δₗ * x_{16*l+10}],
         where δₗ (delta_weights) are dynamic, contrast-enhanced global weights.
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        for l in range(5):
            ...
            # Use the same auxiliary features as in Stage 1
            for j in range(self.n_qubits):
                idx_a = base + 10 + (j % 6)
                idx_b = base + 10 + ((j + 1) % 6)
                x_a = x[idx_a]
