...
class DynamicCPSMidRangeWithAdaptiveGlobalFusionFeatureMap(BaseFeatureMap):
    """
    Dynamic CPS Mid-Range with Adaptive Global Fusion Feature Map.
    
    This feature map partitions an 80-dimensional PCA-reduced input into 5 layers (each with 16 features).
    For each layer (l = 0,...,4):
      
    - Local Encoding:
         The first 10 features are encoded onto a 10-qubit register via fixed RY(π*x) rotations to retain local amplitude information.
    ...
    - Stage 1 (Immediate Neighbor Entanglement):
         ...
         A CRX gate is applied between qubit j and its immediate neighbor, with rotation angle computed as:
             π * (0.5*x_a + 0.5*x_b + ε*(x_a - x_b)),
         where ε is an adaptively tuned contrast coefficient (default 0.05).
      
    - Stage 2 (Next-Nearest Neighbor Entanglement):
         For each qubit j, three auxiliary features (indices: (j mod 6), ((j+2) mod 6), and ((j+4) mod 6), offset by 10) are averaged.
    ...
    - Stage 3 (Mid-Range Entanglement via CPS):
         For each qubit j, using the same pair of auxiliary features as in Stage 1, a CPS (ControlledPhaseShift) gate is applied between qubit j and
         the qubit three positions ahead, with rotation angle given by:
             π * λₗ * (0.5*x_a + 0.5*x_b),
         where λₗ is an adaptive scaling factor for the layer (provided via lambda_values).
      
    - Global Fusion:
         After processing all layers, a MultiRZ gate aggregates selected inter-layer features with a rotation angle:
             π * Σₗ (δₗ * x_{16*l+10}),
