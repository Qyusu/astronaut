...
class DualAxisWeightedEncodingWithIssingXXAugmentedEntanglementFeatureMap(BaseFeatureMap):
    """
    Dual-Axis Weighted Encoding with IssingXX Augmented Entanglement Feature Map.
    ...
    For each layer l (l = 1,...,5):
      ...
      - Local Encoding: The first 10 features are embedded onto a 10-qubit register using a two-step rotation:
          1. RY(π * x[16*(l-1)+j]) to map amplitude information,
          2. RX((π/2) * x[16*(l-1)+j]) to provide additional axis information.
      
      - Stage 1 (Immediate Neighbor Entanglement Augmented with IsingXX):
          For each qubit j, compute the following from the entanglement features:
            a = x[16*(l-1) + 10 + (j mod 6)]
            b = x[16*(l-1) + 10 + ((j+1) mod 6)]
          Then calculate:
            avg = 0.5 * a + 0.5 * b
            diff = a - b
            angle = π * (avg + 0.3 * diff)
          Apply a controlled rotation between qubit j and (j+1) mod n_qubits using:
            - CRZ if the layer is odd
            - CRX if the layer is even
          In addition, an IsingXX gate is applied between the same qubits with rotation angle π * 0.4 * diff.
      
      - Stage 2 (Next-Nearest Neighbor Entanglement):
          For each qubit j, compute a triple average from indices (j mod 6), ((j+2) mod 6) and ((j+4) mod 6)
          in the block starting at 16*(l-1) + 10. The rotation angle is given by π times this average,
          and a CRY gate is applied between qubit j and (j+2) mod n_qubits.
      
      - Stage 3 (Mid-Range Entanglement with Adaptive Calibration):
          For each qubit j, compute the average of two entanglement features:
            avg_pair = 0.5 * x[16*(l-1) + 10 + (j mod 6)] + 0.5 * x[16*(l-1) + 10 + ((j+1) mod 6)]
          Scale this average by an adaptive factor λₗ and apply a CRot gate (with theta=0 and omega=0) between
          qubit j and (j+3) mod n_qubits with rotation angle π * λₗ * avg_pair.
      
      - Global Entanglement:
          A MultiRZ gate is applied across all qubits with rotation angle π times the weighted sum of a designated
          feature from each layer:
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        for l in range(5):
            ...
            # Stage 1: Immediate Neighbor Entanglement with Augmented IsingXX
            for j in range(self.n_qubits):
                idx_a = base + 10 + (j % 6)
                idx_b = base + 10 + ((j + 1) % 6)
                avg = 0.5 * x[idx_a] + 0.5 * x[idx_b]
                diff = x[idx_a] - x[idx_b]
                angle_crot = np.pi * (avg + 0.3 * diff)
                if (l + 1) % 2 == 1:
                    qml.CRZ(phi=angle_crot, wires=[j, (j + 1) % self.n_qubits])
                else:
                    qml.CRX(phi=angle_crot, wires=[j, (j + 1) % self.n_qubits])
                
                # Additional IsingXX gate to enhance phase encoding
                angle_isingxx = np.pi * 0.4 * diff
            ...
            # Stage 3: Mid-Range Entanglement with Adaptive Calibration via CRot gates
            for j in range(self.n_qubits):
                idx_a = base + 10 + (j % 6)
                idx_b = base + 10 + ((j + 1) % 6)
