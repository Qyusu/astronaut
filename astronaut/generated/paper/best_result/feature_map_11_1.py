...
class WeightedDualStagePlusModeratedMidRangeOptimizedGlobalFusionFeatureMap(BaseFeatureMap):
    """
    Weighted Dual-Stage Plus Moderated Mid-Range and Optimized Global Fusion Feature Map.
    ...
    For each layer l (l = 0,...,4):
      ...
      - Stage 1 (Immediate neighbor entanglement):
            ...
            then apply a controlled rotation between qubit j and (j+1) mod n_qubits using:
              - CRZ if the layer index (l+1) is odd
              - CRX if the layer index (l+1) is even
      ...
      - Stage 3 (Mid-range entanglement):
            For each qubit j, compute
              θ₃ = u_stage3[l][j] * x[16*l + 10 + (j mod 6)] + (1 - u_stage3[l][j]) * x[16*l + 10 + ((j+1) mod 6)],
            scale it by lambda_factor, and apply a ControlledPhaseShift gate between qubit j and (j+3) mod n_qubits with rotation angle π·lambda_factor·θ₃.
      - Global entanglement: After processing all layers, a MultiRZ gate is applied across all qubits with rotation angle
            π * (Σₗ δₗ * x[16*l + 10]),
            where δₗ are optimized non-uniform weights provided in delta_weights.
    ...
    def __init__(self, 
        ...
        """
        Initialize the Weighted Dual-Stage Plus Moderated Mid-Range and Optimized Global Fusion Feature Map.
        ...
        Args:
            ...
            u_stage3 (list): 5x10 weight matrix for Stage 3 (default: all 0.5).
            lambda_factor (float): Scaling factor for Stage 3 (default: 0.5, with 0 < lambda_factor < 1).
            delta_weights (list): List of 5 optimized weights for global entanglement (default: [0.15, 0.25, 0.35, 0.15, 0.10]).
        """
        ...
        if u_stage3 is None:
            self.u_stage3 = [[0.5 for _ in range(n_qubits)] for _ in range(5)]
        else:
            self.u_stage3 = u_stage3
    ...
    def feature_map(self, x: np.ndarray) -> None:
        """
        Create the quantum circuit for the Weighted Dual-Stage Plus Moderated Mid-Range and Optimized Global Fusion Feature Map.
        ...
        for l in range(5):
            ...
            for j in range(self.n_qubits):
                ...
                angle_stage1 = np.pi * theta1
                # Use CRZ for odd layers (l+1 odd), CRX for even layers
                if (l + 1) % 2 == 1:
            ...
            for j in range(self.n_qubits):
                idx_a = base + 10 + (j % 6)
                idx_b = base + 10 + ((j + 1) % 6)
                theta3 = self.u_stage3[l][j] * x[idx_a] + (1 - self.u_stage3[l][j]) * x[idx_b]
                angle_stage3 = np.pi * self.lambda_factor * theta3
                qml.ControlledPhaseShift(phi=angle_stage3, wires=[j, (j + 3) % self.n_qubits])
