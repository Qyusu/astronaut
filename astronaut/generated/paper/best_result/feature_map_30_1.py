...
class DualAxisEncodingWithNoiseCalibratedCPSMidRangeAndDynamicGlobalFusionFeatureMap(BaseFeatureMap):
    """
    Dual-Axis Encoding with Noise-Calibrated CPS Mid-Range and Dynamic Global Fusion Feature Map.
    
    This feature map partitions an 80-dimensional PCA‑reduced input into 5 layers (each with 16 features).
    For each layer (l = 0,...,4):
      
    - Local Encoding:
         The first 10 features are encoded onto a 10-qubit register using fixed RY(π·x) rotations to linearly map amplitudes,
         followed by fixed RZ((π/3)·x) rotations to diversify the phase without introducing nonlinearity.
         This dual-axis encoding ensures a robust representation of both amplitude and phase.
      
    - Stage 1 (Immediate Neighbor Entanglement):
         For each qubit j, two entanglement features are selected from indices (j mod 6) and ((j+1) mod 6) offset by 10.
         ...
         where ε is a fixed contrast parameter obtained via noise calibration (default 0.1).
    ...
    - Stage 3 (Mid-Range Entanglement via CPS):
         For each qubit j, using the same pair of features as in Stage 1, a ControlledPhaseShift gate is applied between qubit j and
         the qubit three positions ahead. The rotation angle is defined as:
             π * λₗ * (0.5*x_a + 0.5*x_b),
         where λₗ is an adaptive scaling factor for the layer (default provided via lambda_values), computed from noise metrics.
      
    - Global Fusion:
         After processing all layers, a MultiRZ gate aggregates inter-layer features with a rotation angle:
             π * Σₗ (δₗ * x_{16*l+10}),
         where δₗ (delta_weights) are dynamically adaptive global fusion weights updated based on current separability and variance metrics.
    ...
    """
    def __init__(self, n_qubits: int, epsilon: float = 0.1, lambda_values: list = None, delta_weights: list = None) -> None:
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        for l in range(5):
            ...
            # Local Encoding: Apply RY followed by RZ rotations for qubits 0 to 9
            ...
            # Stage 3: Mid-Range Entanglement using ControlledPhaseShift (CPS) gates
            for j in range(self.n_qubits):
                idx_a = base + 10 + (j % 6)
                idx_b = base + 10 + ((j + 1) % 6)
                x_a = x[idx_a]
