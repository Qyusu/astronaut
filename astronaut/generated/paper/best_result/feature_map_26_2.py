...
class DynamicMergedEntanglementWithIssingXXAugmentationFeatureMap(BaseFeatureMap):
    ...
    - IssingXX Augmentation:
         To enhance mid-range phase encoding while preserving linearity, an IssingXX gate is applied on the same qubit pairs as in the immediate entanglement stage.
         The rotation angle is computed as:
             π * γ * |x_a - x_b|,
         where γ is a fixed scaling constant (default 0.1) and the absolute difference |x_a - x_b| is used to strengthen phase correlations.
