...
class AdaptiveSingleAxisWithMinimalMidRangeAndDynamicGlobalFusionFeatureMap(BaseFeatureMap):
    """
    Adaptive Single-Axis with Minimal Mid-Range and Dynamic Global Fusion Feature Map.
    ...
    - Stage 3 (Minimal Mid-Range Entanglement):
         For each qubit j, two features from the immediate entanglement block are used to compute an average:
           pair_avg = 0.5 * (x[16*l+10 + (j mod 6)] + x[16*l+10 + ((j+1) mod 6)])
         This average is scaled by an adaptive factor λₗ (provided via lambda_factors) to yield a rotation angle:
           angle = π * λₗ * pair_avg
         A CRot gate (with theta=0.0 and omega=0.0) is applied between qubit j and (j+3) mod n_qubits.
      
    - Global Entanglement:
         Inter-layer correlations are fused via a MultiRZ gate with a rotation angle computed as:
    ...
    def __init__(self, n_qubits: int, lambda_factors: list = None, delta_weights: list = None, eta: float = 0.1) -> None:
        ...
        # Adaptive scaling factors for minimal mid-range entanglement (Stage 3)
        ...
        # Contrast parameter for dynamic global fusion
        self.eta = eta
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        for l in range(5):
            ...
            # Stage 3: Minimal Mid-Range Entanglement using CRot gates
            for j in range(self.n_qubits):
                idx_a = base + 10 + (j % 6)
                idx_b = base + 10 + ((j + 1) % 6)
                pair_avg = 0.5 * (x[idx_a] + x[idx_b])
                angle_cr = np.pi * self.lambda_factors[l] * pair_avg
                qml.CRot(phi=angle_cr, theta=0.0, omega=0.0, wires=[j, (j + 3) % self.n_qubits])
