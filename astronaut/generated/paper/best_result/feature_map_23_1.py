...
class HybridDualStageWithControlledPhaseShiftMidRangeEntanglementFeatureMap(BaseFeatureMap):
    """
    Hybrid Dual-Stage with ControlledPhaseShift Mid-Range Entanglement Feature Map.
    ...
    - Stage 1 (Immediate Neighbor Entanglement):
         ...
         A CRX gate is applied between qubit j and its neighbor (j+1 mod n_qubits) with rotation angle:
             π * (0.5 * x_a + 0.5 * x_b + 0.1 * (x_a - x_b)),
         where x_a and x_b are the selected features.
      
    - Stage 2 (Next-Nearest Neighbor Entanglement):
         For each qubit j, three features are selected from the layer and averaged equally.
         A CRY gate is applied between qubit j and (j+2 mod n_qubits) with rotation angle π times this average.
      
    - Stage 3 (Mid-Range Entanglement via ControlledPhaseShift):
         For each qubit j, two features (similar to Stage 1) are used to compute a rotation angle scaled by an adaptive factor λₗ.
         A ControlledPhaseShift gate is applied between qubit j and (j+3 mod n_qubits) with angle:
             π * λₗ * (0.5 * x_a + 0.5 * x_b).
      
    - Global Fusion:
         Inter-layer features are aggregated via a MultiRZ gate with rotation angle computed as
             π * Σₗ [ δₗ * x_{16*l+10} ],
         where δₗ are non-uniform global weights.
      
    Note: The input x is expected to have shape (80,).
      
    Parameters:
      n_qubits (int): Number of qubits (ideally 10).
      lambda_factors (list): Adaptive scaling factors for the mid-range entanglement stage.
               Default: [0.3, 0.3, 0.3, 0.3, 0.3].
      delta_weights (list): Global entanglement weights.
               Default: [0.15, 0.25, 0.35, 0.15, 0.10].
    """
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        for l in range(5):
            ...
            for j in range(self.n_qubits):
                ...
                x_a = x[idx_a]
                x_b = x[idx_b]
                angle_cps = np.pi * self.lambda_factors[l] * (0.5 * x_a + 0.5 * x_b)
