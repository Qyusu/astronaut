...
class HybridCPSMidRangeWithDynamicGlobalFusionFeatureMap(BaseFeatureMap):
    ...
    - Local Encoding:
         The first 10 features are encoded onto a 10-qubit register using fixed RY(π*x) rotations to preserve essential feature amplitudes.
         (Unlike dual-axis schemes, no additional RZ rotation is applied.)
      
    - Stage 1 (Immediate Neighbor Entanglement):
         For each qubit j, two entanglement features are selected from indices based on (j mod 6) and ((j+1) mod 6), offset by 10.
         ...
         where ε is a contrast coefficient (default 0.1), representing a modest scaling of the feature difference.
    ...
    - Stage 3 (Mid-Range Entanglement via CPS):
         For each qubit j, using the same pair of features as in Stage 1, a ControlledPhaseShift (CPS) gate is applied between qubit j and
         the qubit three positions ahead. The rotation angle is given by:
             π * λₗ * (0.5*x_a + 0.5*x_b),
         where λₗ is an adaptive scaling factor for the layer (default provided via lambda_values).
      
    - Global Fusion:
         After processing all layers, a MultiRZ gate aggregates inter-layer features with a rotation angle:
             π * Σₗ (δₗ * x_{16*l+10}),
         where δₗ (delta_weights) are dynamically adaptive global fusion weights.
