...
class HybridWeightedMultiEntanglementFeatureMapV3(BaseFeatureMap):
    """Hybrid Weighted Multi-Entanglement Feature Map v3.
    ...
    For each qubit:
      ...
      - Secondary entanglement: Apply CRY gates between each qubit and the qubit three positions ahead with rotation angle
      ...
      - Tertiary entanglement: Apply CRX gates between each qubit and the qubit four positions ahead with rotation angle
        ...
        qubit (i+4) mod 10.
      - U_post: A post-entanglement rotation block applies local rotations in the order RY, RZ, RX using angles
        π*f₇, π*f₈, and π*f₇ (with f₇ reused).
    The overall circuit implements |Φ(x)⟩ = (∏ U_post) (∏ CRX) (∏ CRY) (∏ CP) (∏ U_primary)|0⟩.
    """

    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4) -> None:
        """Initialize the Hybrid Weighted Multi-Entanglement Feature Map v3.
    ...
    def feature_map(self, x: np.ndarray) -> None:
        """Create the quantum circuit for the Hybrid Weighted Multi-Entanglement Feature Map v3.
        ...
        The 80 features are divided into 10 blocks of 8 features each. For each qubit i (0 ≤ i ≤ 9):
          ...
          3. Secondary entanglement: Apply a CRY gate between qubit i and qubit (i+3)%n_qubits with angle
             π * ((2 * f₅ + f₅_partner) / 3), where f₅ is the fifth feature of qubit i and f₅_partner is that of
             qubit (i+3)%n_qubits.
          4. Tertiary entanglement: Apply a CRX gate between qubit i and qubit (i+4)%n_qubits with angle
             π * ((2 * f₆ - f₆_partner) / 3), where f₆ is the sixth feature of qubit i and f₆_partner is that of
             qubit (i+4)%n_qubits.
          5. U_post: Apply local rotations in the order RY, RZ, RX with angles π*f₇, π*f₈, π*f₇ (features 7, 8, and 7 reused).
        ...
        # Step 3: Secondary entanglement using CRY gates between qubits separated by three positions
        for qubit in range(self.n_qubits):
            partner = (qubit + 3) % self.n_qubits
            f5_current = x[8 * qubit + 4]
            f5_partner = x[8 * partner + 4]
            angle_cry = np.pi * ((2 * f5_current + f5_partner) / 3.0)
