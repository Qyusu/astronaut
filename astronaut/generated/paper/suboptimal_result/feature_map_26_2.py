...
class HierarchicalBridgingEnhancedFeatureMapV26(BaseFeatureMap):
    """
    Hierarchical Bridging Enhanced Feature Map v26.
    
    This feature map partitions an 80-dimensional normalized input among 10 qubits, dividing them into two sub-blocks
    (Block A: qubits 0–4 and Block B: qubits 5–9). Each qubit first applies a local rotation block U_local^(i) that encodes
    features f₁–f₄ using variable cyclic orders. Within each block, a structured entanglement sequence is executed:
      - A CP gate ring among all qubits in the block,
      - A CRY layer at offset 2 using π*((f₍i,4₎ + f₍(i+2),4₎)/2),
      - A CRZ layer at offset 3 using π*(2f₍i,5₎ + 3f₍(i+3),6₎)/5,
      - A CRX layer at offset 4 using π*((f₍i,2₎ + f₍(i+4),2₎)/2),
      - An additional CRX layer with a tunable offset δ capturing intra-block long-range correlations, 
        computed as π*((f₍i,3₎ + f₍(i+δ),3₎)/6), with δ defined modulo the block size.
    Inter-block bridging is achieved by applying CRZ gates between corresponding qubits in Block A and Block B
    using π*((f₍i,5₎ + f₍(i+5),5₎)/4). Global correlations are integrated via a MultiRZ gate with a rotation angle
    determined by π*(Σ(f₁+f₂+f₃+f₅+f₆+2f₇+2f₈)/97). Finally, each qubit applies a post-entanglement block U_post^(i)
    composed of a fixed cyclic sequence: RZ(π·f₇) → RX(π·f₈) → RY(π·f₇) → RY(π/4).
    Hardware-aware error mitigation, including dynamic calibration and Bit Flip Tolerance, is assumed at the device level.
    """
    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4, global_const: float = 97, delta: int = 6) -> None:
        """Initialize the Hierarchical Bridging Enhanced Feature Map v26.
        ...
        Args:
            ...
            delta (int): Tunable offset parameter for the additional CRX layer (default: 6).
        """
    ...
    def feature_map(self, x: np.ndarray) -> None:
        """Construct the quantum circuit for the Hierarchical Bridging Enhanced Feature Map v26.
        ...
        for qubit in range(self.n_qubits):
            ...
            # Variable cyclic rotation order based on qubit index modulo 5
            if qubit % 5 == 0:
        ...
        for block in [block_A, block_B]:
            # (a) CP gate ring within the block
            ...
            # (e) Additional CRX layer with tunable offset δ using feature f3
            for idx in range(len(block)):
                current = block[idx]
