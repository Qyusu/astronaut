...
class AdaptiveFullyConnectedGlobalFeatureMapV25(BaseFeatureMap):
    """
    Adaptive Fully Connected Global Feature Map v25.
    
    This feature map partitions an 80-dimensional normalized input among 10 qubits (with f(i,j) = x[8*i + j]).
    Each qubit first applies a local rotation block U_local^(i) that encodes features f1–f4 using variable cyclic permutations
    chosen from an expanded fixed set to maximize state diversity. It then implements a fully connected entanglement scheme with layered
    operations:
      - A nearest-neighbor CP gate ring,
      - A CRY layer at offset 2 using a rotation angle π*((f4_current + f4_partner)/2),
      - A CRZ layer at offset 3 with rotation π*(2f5 + 3f6_partner)/5,
      - A CRX layer at offset 4 using a rotation angle π*((f2_current + f2_partner)/2),
      - An extra CRX layer at offset 6 with rotation angle π*((f3_current + f3_partner)/6),
      - An ultra-light CRY layer at offset 5 with rotation angle π*((f8_current + f8_partner)/28).
    Global interactions are integrated via a MultiRZ gate with rotation angle π*(Σ(f1+f2+f3+f5+f6+2f7+2f8)/98).
    Finally, each qubit applies a post-entanglement block U_post^(i) with a fixed cyclic sequence: RZ(π·f7) → RY(π·f8) → RX(π·f7).
    Hardware-aware error mitigation is assumed to be applied at the device level.
    """
    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4, global_const: float = 98) -> None:
        """Initialize the Adaptive Fully Connected Global Feature Map v25.
        ...
        Args:
            ...
            global_const (float): Normalization constant for the MultiRZ gate (default: 98).
        """
    ...
    def feature_map(self, x: np.ndarray) -> None:
        """Construct the quantum circuit for the Adaptive Fully Connected Global Feature Map v25.
        ...
        for qubit in range(self.n_qubits):
            ...
            # Choose cyclic rotation order based on qubit index modulo 4
            if qubit % 4 == 0:
        ...
        # Step 7: Ultra-light CRY layer at offset 5 using feature f8
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
            f8_current = x[base_idx + 7]
            partner = (qubit + 5) % self.n_qubits
            f8_partner = x[8 * partner + 7]
            ultra_cry_angle = np.pi * ((f8_current + f8_partner) / 28.0)
