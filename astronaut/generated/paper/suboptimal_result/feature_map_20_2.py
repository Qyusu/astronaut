...
class RecalibratedTwoBlockHybridFeatureMapV20(BaseFeatureMap):
    ...
    Each qubit first applies a deterministic U_local block that encodes features f₁–f₄ using a fixed cyclic order:
      U_local^(i) = RX(π·f₁) → RY(π·f₂) → RZ(π·f₃) → RX(π·f₄).
    The entanglement layers are structured as follows:
    ...
    (f₁ + f₂ + f₃ + f₅ + f₆ + 2f₇ + 2f₈) divided by 95. Finally, a post‐entanglement block U_post applies a fixed cyclic
    rotation sequence: U_post^(i) = RZ(π·f₇) → RX(π·f₈) → RY(π·f₇).
    """
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        """
        # Step 1: U_local block - encode features f₁ to f₄ using a fixed cyclic order
        for qubit in range(self.n_qubits):
            ...
            # Fixed rotation order for all qubits
