...
class EnhancedMultiRZUltraLightCRYFeatureMapV21(BaseFeatureMap):
    ...
    permutations chosen from an expanded set to ensure diverse state preparation. Following this, a layered entanglement
    structure is applied:
      - A nearest-neighbor ControlledPhaseShift (CP) gate ring entangles adjacent qubits.
      - A CRY layer at offset 2 connects qubits using rotation angles π·((f₄_current + f₄_partner)/2).
      - A CRZ layer at offset 3 applies rotations with angle π·(2f₅ + 3f₆_partner)/5.
      - A CRX layer at offset 4 couples qubits via the average of f₂.
      - An extra CRX layer at offset 6 applies a reduced rotation π·((f₃_current + f₃_partner)/6) to capture long-range correlations.
      - An ultra-light CRY layer at offset 5 uses a very low rotation angle π·((f₈_current + f₈_partner)/32) to further enrich connectivity.
    Global correlations are consolidated via a MultiRZ gate with rotation angle π multiplied by the sum over all
    qubits of (f₁ + f₂ + f₃ + f₅ + f₆ + 2f₇ + 2f₈) divided by 95. Finally, a post-entanglement block U_post applies
    a fixed cyclic sequence: U_post^(i) = RZ(π·f₇) → RY(π·f₈) → RX(π·f₇).
    """
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        """
        # Step 1: U_local block - encode features f₁ to f₄ using variable cyclic permutations
        for qubit in range(self.n_qubits):
            ...
            # Select a cyclic rotation order based on qubit index (using mod 3 as an example)
            if qubit % 3 == 0:
        ...
        # Step 7: Ultra-light CRY layer at offset 5 using the average of feature f₈ with scaling denominator 32
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
        ...
        # Step 9: U_post block - apply fixed cyclic rotations: RZ, RY, RX on features f₇ and f₈
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
            f7 = x[base_idx + 6]
            f8 = x[base_idx + 7]
