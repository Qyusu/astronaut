...
class RefinedTwoBlockOptimizedGlobalFeatureMapV18(BaseFeatureMap):
    """
    Refined Two-Block Optimized Global Feature Map v18.
    
    This feature map partitions an 80-dimensional normalized input among 10 qubits (with f_{i,j} = x[8*i + j]).
    A separate local rotation block U_local encodes features f1–f4 via variable cyclic orders chosen from an expanded pool,
    ensuring that all rotations remain linear functions of the input features. Nearest-neighbor entanglement is established
    via a CP gate ring, followed by a CRY layer at offset 2 (using the average of feature f4), a CRZ layer at offset 3
    (with π·(2f5+3f6_partner)/5), a CRX layer at offset 4 (using the average of feature f2), and an extra CRX layer at offset 6
    (applying π·((f3+f3_partner)/6)) to capture long‐range correlations. Global correlations are then integrated via a
    MultiRZ gate with rotation angle π multiplied by the sum of selected features (f1+f2+f3+f5+f6+2f7+2f8) divided by 95.
    Finally, a post‐entanglement block U_post applies a fixed cyclic sequence of rotations (RZ, RX, RY on f7 and f8).
    ...
    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4) -> None:
        """Initialize the Refined Two-Block Optimized Global Feature Map v18.
    ...
    def feature_map(self, x: np.ndarray) -> None:
        ...
        """
        # Step 1: U_local block - encode features f1 to f4 with a variable cyclic order
        ...
        # Step 8: U_post block - apply fixed cyclic rotations based on features f7 and f8
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
            f7 = x[base_idx + 6]
            f8 = x[base_idx + 7]
