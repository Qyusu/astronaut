...
class HierarchicalBridgingEnhancedAdaptiveFeatureMapV27Dynamic(BaseFeatureMap):
    """
    Hierarchical Bridging Enhanced Adaptive Feature Map v27-Dynamic.
    ...
    using a dynamically chosen cyclic permutation from an expanded fixed pool. Within each sub-block, intra-block entanglement is applied via:
      - A CP gate ring connecting nearest neighbors within the block.
      - A CRY layer at offset 2 with rotations π·((f(i,4) + f((i+2),4))/2).
      - A CRZ layer at offset 3 with rotations π·((2f(i,5) + 3f((i+3),6))/5).
      - A CRX layer at offset 4 with rotations π·((f(i,2)+f((i+4),2))/2).
      - An additional CRX layer at offset 6 (computed modulo the block size) with rotations π·((f(i,3) + f((i+6),3))/6).
    Inter-block bridging is achieved via CRZ gates connecting corresponding qubits from Block A and Block B using rotation angles
    π·((f(i,5)+f((i+5),5))/4). Global correlations are integrated via a MultiRZ gate with angle π * (Σ(f(i,1)+f(i,2)+f(i,3)+f(i,5)+f(i,6)+2f(i,7)+2f(i,8)))/97.
    Finally, the U_post block applies a fixed cyclic sequence of rotations: RZ(π·f(i,7)) → RX(π·f(i,8)) → RY(π·f(i,7)) followed by an extra RY(π/4).
    
    All gate parameters are linear functions of the input features. Dynamic selection of the local rotation orders
    and modular arithmetic within blocks are employed to ensure a rich entanglement structure and reduced circuit depth.
    """
    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4, global_const: float = 97) -> None:
        """Initialize the Hierarchical Bridging Enhanced Adaptive Feature Map v27-Dynamic.
        ...
        self.block_size = n_qubits // 2  # Divide 10 qubits into two blocks of 5 qubits each

    def feature_map(self, x: np.ndarray) -> None:
        """Construct the quantum circuit for the Hierarchical Bridging Enhanced Adaptive Feature Map v27-Dynamic.
        ...
        # Step 1: U_local block - encode features f1 to f4 using a dynamic cyclic permutation for all qubits
        ...
        # Step 3: Inter-block bridging using CRZ gates between corresponding qubits of Block A and Block B
        ...
        # Step 5: U_post block - apply fixed cyclic rotations followed by an extra RY(π/4)
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
            f7 = x[base_idx + 6]
            f8 = x[base_idx + 7]
