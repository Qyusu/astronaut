...
class BalancedExtendedEntanglementFeatureMapV12(BaseFeatureMap):
    """Balanced Extended Entanglement Feature Map v12.
    ...
    A unified local rotation block U_unified^(i) encodes features f₁, f₂, f₃, f₄, f₇, and f₈ in a single sequence using a unique cyclic order selected from an expanded set.
    The entanglement layers include a nearest-neighbor CP gate ring; a CRY layer (offset 2) based on the average of f₄; a CRZ layer (offset 3) using f₅ and f₆; a CRX layer (offset 4) using the average of f₂; and an extra CRX layer (offset 6) with a scaled average of f₃.
    Global correlations are captured via a MultiRZ gate with rotation angle π * (global sum/105).
    """
    
    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4) -> None:
        """Initialize the Balanced Extended Entanglement Feature Map v12.
    ...
    def feature_map(self, x: np.ndarray) -> None:
        """Create the quantum circuit for the Balanced Extended Entanglement Feature Map v12.
        
        Args:
            x (np.ndarray): Input feature vector of shape (80,).
        """
        # Step 1: U_unified - Unified local rotation block encoding features f1, f2, f3, f4, f7, and f8
        for qubit in range(self.n_qubits):
            ...
            # Select a unique cyclic permutation from an expanded set based on qubit index modulo 3
            if qubit % 3 == 0:
        ...
        # Step 6: Extra entanglement using an additional CRX gate (offset 6) with a scaled average of f3
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
            f3_current = x[base_idx + 2]
            partner = (qubit + 6) % self.n_qubits
            f3_partner = x[8 * partner + 2]
            extra_crx_angle = np.pi * ((f3_current + f3_partner) / 4.0)
