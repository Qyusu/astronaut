...
class UnifiedMultiPermutationExpandedGlobalFeatureMapV11(BaseFeatureMap):
    """Unified Multi-Permutation Expanded Global Feature Map v11.
    
    This feature map assigns an 80-dimensional normalized input evenly to 10 qubits (each qubit i receives features f_{i,j} = x[8*i + j] for j = 0,...,7).
    A unified rotation block U_unified^(i) encodes features f₁, f₂, f₃, f₄, f₇, and f₈ using a fixed cyclic permutation selected from an expanded set.
    The entanglement structure is arranged as follows:
      - Nearest-neighbor entanglement: A ring of ControlledPhaseShift (CP) gates connects qubit i with qubit (i+1) mod 10.
      - Additional entanglement 1: A CRY layer with an offset of 2 is applied using feature f₄ (average of f₍i,4₎ and f₍(i+2),4₎).
      - Intermediate entanglement: CRZ gates connect qubit i with qubit (i+3) mod 10 using rotation angle π·(2·f₍i,5₎ + 3·f₍(i+3),6₎)/5.
      - Novel entanglement: A CRX layer connects qubit i with qubit (i+4) mod 10 using the average of feature f₂.
      - Extra entanglement: An additional CRY layer with an offset of 6 is applied using feature f₃ (average of f₍i,3₎ and f₍(i+6),3₎).
      - Global entanglement: A MultiRZ gate integrates correlations with rotation angle π·(Σᵢ(f₁+f₂+f₃+f₅+f₆+2f₇+2f₈))/100.
    The overall operation is given by:
      |Φ(x)⟩ = MultiRZ(π·(Σᵢ(f₁+f₂+f₃+f₅+f₆+2f₇+2f₈))/100) · (∏ CRY_{i,(i+6) mod 10}(π·((f₃+f₍i+6₎,3)/2))) ·
               (∏ CRX_{i,(i+4) mod 10}(π·((f₂+f₍i+4₎,2)/2))) · (∏ CRZ_{i,(i+3) mod 10}(π·(2f₍i,5₎+3f₍(i+3),6₎)/5)) ·
               (∏ CRY_{i,(i+2) mod 10}(π·((f₍i,4₎+f₍(i+2),4₎)/2))) · (∏ CP(q_i, q_{(i+1) mod 10})) · (∏ U_unified^(i)) |0⟩,
    where
      U_unified^(i) = R_{β_{i,1}}(π·f₍i,1₎) R_{β_{i,2}}(π·f₍i,2₎) R_{β_{i,3}}(π·f₍i,3₎)
                      R_{β_{i,4}}(π·f₍i,4₎) R_{β_{i,5}}(π·f₍i,7₎) R_{β_{i,6}}(π·f₍i,8₎), with f₍i,j₎ = x[8*i + j].
    """
    
    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4) -> None:
        """Initialize the Unified Multi-Permutation Expanded Global Feature Map v11.
    ...
    def feature_map(self, x: np.ndarray) -> None:
        """Create the quantum circuit for the Unified Multi-Permutation Expanded Global Feature Map v11.
        ...
        for qubit in range(self.n_qubits):
            ...
            # Select a cyclic permutation from an expanded set based on qubit index modulo 3
            if qubit % 3 == 0:
        ...
        # Step 6: Extra entanglement using an additional CRY gate (offset of 6) with f3
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
            f3_current = x[base_idx + 2]
            partner = (qubit + 6) % self.n_qubits
            f3_partner = x[8 * partner + 2]
            cry_extra_angle = np.pi * ((f3_current + f3_partner) / 2.0)
            qml.CRY(phi=cry_extra_angle, wires=[qubit, partner])
