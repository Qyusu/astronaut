...
class RecalibratedTwoBlockHybridFeatureMapV20(BaseFeatureMap):
    """
    Recalibrated Two-Block Hybrid Feature Map v20.
    
    This feature map partitions an 80-dimensional normalized input among 10 qubits (with f_{i,j} = x[8*i + j]).
    Each qubit applies a local rotation block U_local^(i) that encodes features f₁–f₄ using variable cyclic orders drawn
    from an expanded pool. Following this, the entanglement sequence is as follows:
    ...
    Global correlations are integrated via a MultiRZ gate with rotation angle π multiplied by the sum over all qubits
    of (f₁ + f₂ + f₃ + f₅ + f₆ + 2f₇ + 2f₈) divided by 95. Finally, a post-entanglement block U_post applies a fixed cyclic
    sequence for features f₇ and f₈, enhanced by an additional fixed RY(π/4) rotation.
    """
    def __init__(self, n_qubits: int, cp_angle: float = np.pi/4) -> None:
        """Initialize the Recalibrated Two-Block Hybrid Feature Map v20.
    ...
    def feature_map(self, x: np.ndarray) -> None:
        """Construct the quantum circuit for the Recalibrated Two-Block Hybrid Feature Map v20.
        
        Args:
            x (np.ndarray): Input normalized feature vector of shape (80,).
        """
        # Step 1: U_local block - encode features f₁ to f₄ using variable cyclic orders
        for qubit in range(self.n_qubits):
            ...
            # Select a variable cyclic rotation order based on qubit index (using mod 2 as an example)
            if qubit % 2 == 0:
        ...
        # Step 8: U_post block - apply fixed cyclic rotations with an additional fixed RY(π/4) rotation
        for qubit in range(self.n_qubits):
            base_idx = 8 * qubit
            f7 = x[base_idx + 6]
            f8 = x[base_idx + 7]
