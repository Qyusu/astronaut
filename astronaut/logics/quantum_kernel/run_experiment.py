...
from qxmt.experiment.schema import RunTime
...
from astronaut.logics.quantum_kernel import generate_feature_map
from astronaut.prompts.quantum_kernel import ReviewIdeaPrompt
...
def update_score_histories(score_histories: str, generate_results: GeneratedResults) -> str:
    ...
    for result in generate_results.results:
        score_histories = template.format(
            ...
            feature_map_name=result.idea.name,
...
def update_config(
    ...
    config = yaml.safe_load(open(BASE_CONFIG_PATH.replace(MODEL_TYPE_PLACEHOLDER, QKERNEL_MODEL_TYPE)))
...
def initialize_run_context(
    ...
    return RunContext(
        model_type=QKERNEL_MODEL_TYPE,
        gen_config_dirc=gen_config_dirc,
        gen_code_dirc=gen_code_dirc,
        model_versions=model_versions,
        n_qubits=get_n_qubits_from_config(BASE_CONFIG_PATH.replace(MODEL_TYPE_PLACEHOLDER, QKERNEL_MODEL_TYPE)),
...
def run(
    ...
    Args:
        ...
        arxiv_db_client (PineconeClient | None):
            Client for accessing academic paper database. If None, the arxiv DB is not used.
        qml_db_client (PineconeClient | None):
            Client for accessing Pennylane documentation database. If None, the qml DB is not used.
    ...
    try:
        for i in range(max_trial_num):
            ...
            if context.need_idea_review:
                performance_review = performance_reviewer.review(
                    context.eval_result_df, context.score_list, ReviewMetric.ACCURACY
                )
            ...
            for i, result in enumerate(generate_first_results.results, start=1):
                ...
                try:
                    ...
                    validated_result, validation_cost = validate_generated_code(
                        ...
                        dry_run_config_path=DRYRUN_CONFIG_PATH.replace(MODEL_TYPE_PLACEHOLDER, QKERNEL_MODEL_TYPE),
                    )
                    ...
                    run_time = cast(RunTime, experiment.exp_db.runs[-1].runtime)  # type: ignore
