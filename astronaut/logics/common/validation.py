...
from astronaut.logics.common.code import save_generated_code
from astronaut.logics.common.generation import GenerateCode
from astronaut.prompts.pennylane_validator import PennyLaneDocsValidatePrompt
from astronaut.prompts.quantum_kernel import RetryGenerateFeatureMapCodePrompt
...
class CodeValidator:
    ...
    def __init__(self, code: str, llm_client: ChatClient, model_version: str, db_client: PineconeClient | None) -> None:
    ...
    def validate_by_pennylane_doc(
        ...
        if self.pennylane_db is None:
            raise ValueError(
                "Validation by Pennylane documentation is not available because the database is not provided."
            )
        ...
        response = self.llm_client.parse_chat(
        ...
        validation_list = self._format_docs_validation_result(response.content)
    ...
    def validate_all(
        ...
        if self.pennylane_db is not None:
            self.validate_by_pennylane_doc(docs_type=docs_type, docs_top_k=docs_top_k, strict=strict)
        else:
            logger.info("Pennylane documentation validation is not conducted because the database is not provided.")
...
@traceable(tags=["validation", "code"])
def dry_run(
    model_type: str, experiment: qxmt.Experiment, config_path: str, generate_result: GeneratedResult
) -> tuple[str, list[str]]:
    ...
    if model_type == QKERNEL_MODEL_TYPE:
...
def validate_generated_code(
    ...
    Args:
        llm_client (ChatClient): Client for interacting with the language model
        db_client (PineconeClient | None): Client for Pinecone database
        ...
        dry_run_config_path (str, optional): Path to dry run config.
    ...
    for _ in range(max_retry):
        ...
        error_massage, warning_messages = dry_run(
            ...
            generate_result=validate_result,
        )
        if error_massage == "":
            break
        else:
