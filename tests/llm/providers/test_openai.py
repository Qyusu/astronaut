...
from astronaut.llm.config import OpenAIConfig
...
def openai_chat_client() -> OpenAIChatClient:
    """Fixture for initializing the OpenAIChatClient with a config."""
    config = OpenAIConfig(
        api_key="test_api_key",
        default_model_version="gpt-4o-2024-11-20",
        reasoning_effort="high",
    )
    return OpenAIChatClient(**config.to_dict())
...
def test_parse_chat_gpt(mocker: MockFixture, openai_chat_client: OpenAIChatClient, mock_sleep: None) -> None:
    ...
    mock_chat = mocker.patch("astronaut.llm.providers.openai.OpenAIChatClient._chat")
    mock_message = ChatCompletionMessage(role="assistant", content="Test response")
    mock_choice = Choice(message=mock_message, finish_reason="stop", index=0)
    mock_completion = ChatCompletion(
        id="test-id",
...
def test_parse_chat_reasoning(mocker: MockFixture, openai_chat_client: OpenAIChatClient, mock_sleep: None) -> None:
    ...
    mock_chat_reasoning = mocker.patch("astronaut.llm.providers.openai.OpenAIChatClient._chat_reasoning_model")
    mock_message = ChatCompletionMessage(role="assistant", content="Test response")
    mock_choice = Choice(message=mock_message, finish_reason="stop", index=0)
    mock_completion = ChatCompletion(
        id="test-id",
