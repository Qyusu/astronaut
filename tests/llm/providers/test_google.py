...
from astronaut.llm.config import GoogleConfig
from astronaut.llm.providers.google import GoogleChatClient
...
@pytest.fixture
def google_chat_client() -> GoogleChatClient:
    """Fixture for initializing the GoogleChatClient with a config."""
    config = GoogleConfig(
        api_key="test_api_key",
        default_model_version="gemini-2.0-flash-001",
        project_id=None,
    )
    return GoogleChatClient(**config.to_dict())
